...
from vajra.nn.modules import VajraStemBlock, VajraV2StemBlock, VajraStambh, VajraStambhV2, ADown, VajraMerudandaBhag1, VajraMerudandaBhag7, VajraGrivaBhag1, VajraMBConvBlock, VajraConvNeXtBlock, Sanlayan, ChatushtayaSanlayan, ConvBNAct, MaxPool, ImagePoolingAttention, VajraWindowAttnBottleneck, VajraV2BottleneckBlock
...
"""class VajraV2Model(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.from_list = [-1, -1, -1, -1, -1, -1, -1, -1, [1, 3, 5, -1], [1, 3, 5, -1], -1, [1, 5, 3, -1], -1, -1, [8, 10, -1], -1, -1, [10, 12, -1], -1, [12, 14, 16]]
        # Backbone
        ...
        self.neck_conv1 = ConvBNAct(channels_list[8], channels_list[8], 1, 3)
        ...
        self.neck_conv2 = ConvBNAct(channels_list[10], channels_list[10], 1, 3)
    ...
    def forward(self, x):
        ...
        pyramid_pool_neck1 = self.pyramid_pool_neck1([pyramid_pool_backbone, vajra_neck1, neck_conv1])
...
class VajraV2Model(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.from_list = [-1, -1, -1, -1, -1, -1, -1, -1, [1, 3, 5, -1], [1, 3, 5, -1], -1, [1, 5, 3, -1], -1, [3, 5, 1, -1], -1, [3, 1, 0,-1], -1, [8, 10, -1], -1, [10, 12, -1], -1, [16, 14, 12, 18, 20]]
        # Backbone
        self.stem = VajraStambh(in_channels, channels_list[0], channels_list[1])
        self.vajra_block1 = VajraMerudandaBhag1(channels_list[1], channels_list[1], num_repeats[0], True, 3, False, 0.5, False) # stride 4
        self.pool1 = ADown(channels_list[1], channels_list[2]) #MaxPool(kernel_size=2, stride=2)
        self.vajra_block2 = VajraMerudandaBhag1(channels_list[2], channels_list[2], num_repeats[1], True, 1, False, 0.5, False) # stride 8
        self.pool2 = ADown(channels_list[2], channels_list[3]) #MaxPool(kernel_size=2, stride=2)
        self.vajra_block3 = VajraMerudandaBhag1(channels_list[3], channels_list[3], num_repeats[2], True, 1, False, 0.5, False) # stride 16
        self.pool3 = ADown(channels_list[3], channels_list[4]) #MaxPool(kernel_size=2, stride=2)
        self.vajra_block4 = VajraMerudandaBhag1(channels_list[4], channels_list[4], num_repeats[3], True, 1, False, 0.5, False) # stride 32
        self.pyramid_pool = Sanlayan(in_c=[channels_list[1], channels_list[2], channels_list[3], channels_list[4]], out_c=channels_list[4], stride=2, use_cbam=False, expansion_ratio=1.0)
        # Neck
        self.fusion4cbam = ChatushtayaSanlayan(in_c=channels_list[1:5], out_c=channels_list[6], use_cbam=False, expansion_ratio=0.5)
        ...
        self.fusion4cbam2 = ChatushtayaSanlayan(in_c=[channels_list[1], channels_list[2], channels_list[3], channels_list[6]], out_c=channels_list[8], use_cbam=False, expansion_ratio=0.5)
        self.vajra_neck2 = VajraGrivaBhag1(channels_list[8], num_repeats[5], 1, 0.5, False, True) # P3, stride=8

        self.fusion4cbam3 = ChatushtayaSanlayan(in_c=[channels_list[2], channels_list[3], channels_list[1], channels_list[8]], out_c=channels_list[8], use_cbam=False, expansion_ratio=0.5)
        self.vajra_neck3 = VajraGrivaBhag1(channels_list[8], num_repeats[5], 1, 0.5, False, True) # P2, stride=4

        self.pyramid_pool_neck1 = Sanlayan(in_c=[channels_list[4], channels_list[6], channels_list[8]], out_c=channels_list[10], stride=2, use_cbam=False, expansion_ratio=0.5)
        self.vajra_neck5 = VajraGrivaBhag1(channels_list[10], num_repeats[6], 1, 0.5, False, True) # P4, stride=16

        self.pyramid_pool_neck2 = Sanlayan(in_c=[channels_list[6], channels_list[8], channels_list[10]], out_c=channels_list[12], stride=2, use_cbam=False, expansion_ratio=0.5)
        self.vajra_neck6 = VajraGrivaBhag1(channels_list[12], num_repeats[7], 1, 0.5, False, True) # P5, stride=32
    ...
    def forward(self, x):
        ...
        fusion4_2 = self.fusion4cbam2([vajra1, vajra3, vajra2, vajra_neck1])
        vajra_neck2 = self.vajra_neck2(fusion4_2) # stride = 8

        fusion4_3 = self.fusion4cbam3([vajra2, vajra3, vajra1, vajra_neck2])
        vajra_neck3 = self.vajra_neck3(fusion4_3) # stride = 4

        pyramid_pool_neck1 = self.pyramid_pool_neck1([pyramid_pool_backbone, vajra_neck1, vajra_neck2])
        vajra_neck5 = self.vajra_neck5(pyramid_pool_neck1) # stride = 16

        pyramid_pool_neck2 = self.pyramid_pool_neck2([vajra_neck1, vajra_neck2, vajra_neck5])
        vajra_neck6 = self.vajra_neck6(pyramid_pool_neck2) # stride = 32

        outputs = [vajra_neck3, vajra_neck2, vajra_neck5, vajra_neck6]
        return outputs
