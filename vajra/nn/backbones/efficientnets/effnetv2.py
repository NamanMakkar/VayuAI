...
from vajra.nn.modules import FusedMBConvEffNet, MBConvEffNet, ConvBNAct, PyramidalPoolCBAM, Fusion4CBAM, VajraBottleneckBlock
...
class EfficientNetV2(nn.Module):
    def __init__(self, 
        ...
        self.blocks = []
        for idx, (channels, num_blocks, stride, expansion_ratio) in enumerate(zip(channels_list, num_repeats, strides, expansion_ratios)):
            channels = make_divisible(channels * width_mul, 8)
            if idx < 3:
                for i in range(num_blocks):
                    self.blocks.append(FusedMBConvEffNet(in_c=block_channels, out_c=channels, stride=stride if i == 0 else 1, expansion_ratio=expansion_ratio))
                    block_channels = channels
            else:
                for i in range(num_blocks):
                    self.blocks.append(MBConvEffNet(in_c=block_channels, out_c=channels, stride=stride if i == 0 else 1, expansion_ratio=expansion_ratio))
                    block_channels = channels
...
class VajraEffNetV2(nn.Module):
    def __init__(self,
        ...
        self.conv1 = ConvBNAct(in_channels, block_channels, stride=2, kernel_size=3)
        self.blocks1 = nn.ModuleList(FusedMBConvEffNet(in_c=block_channels if i == 0 else self.channels_list[0], out_c=self.channels_list[0], stride=strides[0] if i == 0 else 1, expansion_ratio=expansion_ratios[0]) for i in range(num_repeats[0]))
        self.blocks2 = nn.ModuleList(FusedMBConvEffNet(in_c=self.channels_list[0] if i == 0 else self.channels_list[1], out_c=self.channels_list[1], stride=strides[1] if i == 0 else 1, expansion_ratio=expansion_ratios[1]) for i in range(num_repeats[1]))
        self.blocks3 = nn.ModuleList(FusedMBConvEffNet(in_c=self.channels_list[1] if i == 0 else self.channels_list[2], out_c=self.channels_list[2], stride=strides[2] if i == 0 else 1, expansion_ratio=expansion_ratios[2]) for i in range(num_repeats[2]))
        self.blocks4 = nn.ModuleList(MBConvEffNet(in_c=self.channels_list[2] if i == 0 else self.channels_list[3], out_c=self.channels_list[3], stride=strides[3] if i == 0 else 1, expansion_ratio=expansion_ratios[3]) for i in range(num_repeats[3]))
        self.blocks5 = nn.ModuleList(MBConvEffNet(in_c=self.channels_list[3] if i == 0 else self.channels_list[4], out_c=self.channels_list[4], stride=strides[4] if i == 0 else 1, expansion_ratio=expansion_ratios[4]) for i in range(num_repeats[4]))
        self.blocks6 = nn.ModuleList(MBConvEffNet(in_c=self.channels_list[4] if i == 0 else self.channels_list[5], out_c=self.channels_list[5], stride=strides[5] if i == 0 else 1, expansion_ratio=expansion_ratios[5]) for i in range(num_repeats[5]))
        self.blocks7 = nn.Identity() if self.is_small else nn.ModuleList(MBConvEffNet(in_c=self.channels_list[5] if i == 0 else self.channels_list[6], out_c=self.channels_list[6], stride=strides[6] if i == 0 else 1, expansion_ratio=expansion_ratios[6]) for i in range(num_repeats[6]))
        self.pyramidal_pool_cbam = PyramidalPoolCBAM(in_c=self.channels_list[2:], out_c=self.channels_list[-1], stride=2) if self.is_small else PyramidalPoolCBAM(in_c=self.channels_list[-4:], out_c=self.channels_list[-1], stride=2)

        self.fusion4cbam = Fusion4CBAM(in_c=self.channels_list[-4:], out_c=neck_channels_list[0]) if not self.is_small else Fusion4CBAM(in_c=[self.channels_list[-3], self.channels_list[-2], self.channels_list[-1], self.channels_list[-1]], out_c=neck_channels_list[0])
        self.vajra_neck1 = VajraBottleneckBlock(neck_channels_list[0], neck_channels_list[1], num_neck_repeats[0], False, 1, False)

        self.fusion4cbam_1 = Fusion4CBAM(in_c=[self.channels_list[3], self.channels_list[4], self.channels_list[5], neck_channels_list[1]], out_c=neck_channels_list[2])
        self.vajra_neck2 = VajraBottleneckBlock(neck_channels_list[2], neck_channels_list[3], num_neck_repeats[1], False, 1, False)

        self.pyramidal_pool_neck = PyramidalPoolCBAM(in_c=[channels_list[-1], neck_channels_list[1], neck_channels_list[3]], out_c=neck_channels_list[4], stride=2)
        self.vajra_neck3 = VajraBottleneckBlock(neck_channels_list[4], neck_channels_list[5], num_neck_repeats[2], False, 1, False)

        self.pyramidal_pool_neck_1 = PyramidalPoolCBAM(in_c=[neck_channels_list[1], neck_channels_list[3], neck_channels_list[5]], out_c=neck_channels_list[6], stride=2)
        self.vajra_neck4 = VajraBottleneckBlock(neck_channels_list[6], neck_channels_list[7], num_neck_repeats[3], False, 1, False)
...
def build_effnetv2(in_channels,
                   num_classes,
                   size="small",
                   task="classify",
                   kpt_shape=None,
    ...
    if task == "classify":
        ...
        model = EfficientNetV2(in_channels, channels_list, num_repeats, strides, expansion_ratios, width_mul)
        ...
        effnetv2 = nn.Sequential(*layers)
        np_model += np_head

        if verbose:
            LOGGER.info(f"Task: classify; Number of Classes: {num_classes}\n\n\n")
            LOGGER.info("Building EfficientNetV2 ...\n\n")
            ...
            LOGGER.info(f"EfficientNetV2-{size}; Task: classify; Total Parameters: {np_model}\n\n")

        return effnetv2, layers, np_model
        ...
        model = VajraEffNetV2(in_channels, channels_list, num_repeats, strides, expansion_ratios, neck_channels_list=neck_channels_list, num_neck_repeats=num_neck_repeats)
        if task == "detect":
        ...
        if verbose:
            ...
            LOGGER.info("Building VajraEffNetV2 ...\n\n")
            LOGGER.info(f"\n{'index':>3}{'from':>20}{'params':>10}  {'module':<45}{'arguments':<30}\n")
            idx_counter = 0
            #idx = 0
            for i, (name, module) in enumerate(model.named_children()):
                if isinstance(module, nn.ModuleList):
                    for idx, block in enumerate(module):
                        if isinstance(block, MBConvEffNet) or isinstance(block, FusedMBConvEffNet):
                            np = sum(x.numel() for x in block.parameters())
            ...
            LOGGER.info(f"\nBackbone Parameters: {np_model}\n\n")
            LOGGER.info(f"Head Parameters: {np_head}\n\n")
            LOGGER.info(f"VajraEffNetV2 {size}; Task: {task}; Total Parameters: {np_model}\n\n")
        ...
        layers.append(model)
        layers.append(head)
        vajra_effnetv2 = nn.Sequential(*layers)

        return vajra_effnetv2, stride, layers, np_model