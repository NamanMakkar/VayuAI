...
from .modules.encoders import ImageEncoderViT, PromptEncoder, FpnNeck, Hiera, ImageEncoder, MemoryEncoder
...
def build_sam(checkpoint="sam_b.pt"):
    model_builder=None
    checkpoint = str(checkpoint)
    for k in sam_model_map.keys():
        if checkpoint.endswith(k):
            model_builder = sam_model_map.get(k)
    if not model_builder:
        raise FileNotFoundError(f"{checkpoint} is not a supported SAM model. Available models are: \n {sam_model_map.keys()}")

    return model_builder(checkpoint)