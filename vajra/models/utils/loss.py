...
import copy
from vajra.nn.attention_utils import bbox2distance
from vajra.utils import LOGGER
from vajra.utils.dfine_ops import box_cxcywh_to_xyxy, box_iou, generalized_box_iou
...
class VarifocalLoss(nn.Module):
    ...
    def forward(self, pred_score, gt_score, label):
        ...
        with autocast(enabled=True):
            loss = (
                (F.binary_cross_entropy_with_logits(pred_score.float(), gt_score.float(), reduction="none") * weight).mean().sum()
            )
...
class DFINECriterion(nn.Module):
    ...
    def __init__(self, 
        ...
        self.fl = FocalLoss(self.alpha, self.gamma)
        self.vfl = VarifocalLoss(self.alpha, self.gamma)
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        for loss in self.losses:
            ...
            l_dict = {k: l_dict[k] * self.loss_weight_dict[k] for k in l_dict if k in self.loss_weight_dict}
        ...
        if "pre_outputs" in outputs:
            ...
            for loss in self.losses:
                ...
                l_dict = {k: l_dict[k] * self.weight_dict[k] for k in l_dict if k in self.loss_weight_dict}
