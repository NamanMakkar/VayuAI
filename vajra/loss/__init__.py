...
class VajraDetectionLoss:
    ...
    def __call__(self, preds, batch):
        """Calculate the sum of the loss for box, cls and dfl multiplied by batch size."""
        loss = torch.zeros(5, device=self.device)  # box, cls, vfl, focal_loss, dfl
        feats = preds[1] if isinstance(preds, tuple) else preds
        pred_distri, pred_scores = torch.cat([xi.view(feats[0].shape[0], self.num_outputs, -1) for xi in feats], 2).split(
            (self.reg_max * 4, self.num_classes), 1
        )
        ...
        #LOGGER.info(f"target_scores shape: {target_scores.shape}\n")
        #LOGGER.info(f"target labels shape: {target_labels.shape}\n")
        
        # Cls loss
        loss[1] = self.bce(pred_scores, target_scores.to(dtype)).sum() / target_scores_sum  # BCE
        loss[2] = self.vfl(pred_scores, target_scores.to(dtype), target_scores.to(dtype)) / target_scores_sum  # VFL
        loss[3] = self.focal_loss(pred_scores, target_scores.to(dtype)) / target_scores_sum # Focal Loss
        ...
        loss[0] *= self.hyperparms.box  # box gain
        loss[1] *= self.hyperparms.cls  # cls gain
        loss[2] *= self.hyperparms.cls # vfl gain
        loss[3] *= self.hyperparms.cls # focal loss gain
        loss[4] *= self.hyperparms.dfl  # dfl gain
        return loss.sum() * batch_size, loss.detach()
...
class PoseLoss(DetectionLoss):
    def __init__(self, model):
        super().__init__(model)
        head = model.model[-1]
        self.kpt_shape = head.keypoint_shape
