...
from vajra.utils import LOGGER, SETTINGS, TESTS_RUNNING, colorstr
...
try:
    ...
    from vajra.utils.torch_utils import de_parallel, torch
...
def _log_tensorboard_graph(trainer):
    ...
    with warnings.catch_warnings():
        ...
        try:
            ...
            trainer_model_device = next(trainer.model.model.parameters()).device
            #LOGGER.info(f"Model on {device}, Trainer model on device: {trainer_model_device}")
            #LOGGER.info(f"im on {(im.device)}")
            model.eval()
...
def on_pretrain_routine_start(trainer):
    """Initialize  logging with SummaryWriter."""
    if SummaryWriter:
        try:
            global WRITER
