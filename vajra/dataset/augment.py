...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        new_instances = Instances2d(bboxes, segments, keypoints, bbox_format="xyxy", normalized=False)
        # Clip
...
class Albumentations:
    ...
    def __init__(self, p=1.0):
        """Initialize the transform object for bbox formatted params."""
        self.p = p
        self.transform = None
        ...
        try:
            ...
            check_version(A.__version__, "1.0.3", strict=True)  # version requirement
            spatial_transforms = {
                "Affine",
                "BBoxSafeRandomCrop",
                "CenterCrop",
            ...
            self.transform = (
                ...
                if self.contains_spatial else A.Compose(T)
            )
...
class Format:
    ...
    def _format_img(self, img):
        """Format the image for Vajra from Numpy array to PyTorch tensor."""
        if len(img.shape) < 3:
