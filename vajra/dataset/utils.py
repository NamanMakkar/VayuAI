...
from vajra.checks import check_file, is_ascii, check_yaml
from vajra.utils.downloads import download, safe_download, unzip_file
from vajra.ops import segment2box, segments_to_boxes
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                if any(len(x) > 6 for x in lb) and (not keypoint):  # is segment
                    ...
                    lb = np.concatenate((classes.reshape(-1, 1), segments_to_boxes(segments)), 1)  # (cls, xywh)
...
def check_det_dataset(dataset, autodownload=True, add_kpts=False):
    """
    ...
    Args:
        ...
        add_kpts (bool, optional): Whether to add add keypoints for small object detection
    Returns:
    ...
    for k in "train", "val":
        if k not in data:
            ...
            LOGGER.info("WARNING ⚠️ renaming data YAML 'validation' key to 'val' to match Vajra format.")
    ...
    if add_kpts:
        ...
        for i, label in enumerate(labels):
            ...
            with open(label, "w") as f:
                for point, box, cls_id in zip(keypoints, boxes, classes):
                    f.write(f"{cls_id} {box[0]} {box[1]} {box[2]} {box[3]} {point[0]} {point[1]} 1 \n")
        LOGGER.info(f"Success! Keypoints added to Detection Dataset for small object detection")
    return data
