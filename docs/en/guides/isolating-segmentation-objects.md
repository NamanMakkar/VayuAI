...
After performing the [Segment Task](../tasks/segment.md), it is sometimes desirable to extract the isolated objects from the inference results. This guide provides a generic recipe on how to accomplish this using the VayuAI [Predict Mode](../modes/predict.md).
...
2. Now iterate over the results and contours. For workflows that want to save an image to file, the source image `base-name` and the detection `class-label` are retrieved for later use (optional).
...
3. Start with generating a binary mask from the source image and then draw a filled contour onto the mask. This will allow the object to be isolated from the other parts of the image.
    ...
    3. The OpenCV `drawContours()` function expects contours to have a shape `[N, 1, 2]` expand section below for more details.

    <details>
    <summary> Expand to understand what is happening when defining the <code>contour</code> variable. </summary>
    <p>
    - `c.masks.xy` :: Provides the coordinates of the mask contour points in the format `(x, y)`. For more details, refer to the [Mask Section from Predict Mode](../modes/predict.md#masks).
...
4. Next there are 2 options for how to move forward with the image from the point and a subsequent option for each.
    ...
    ??? question "What if I want the cropped object **including** the background?"
        ...
        This is a built in feature. See the `save_crop` argument for [Predict Mode Inference Arguments](../modes/predict.md#inference-arguments) for details.

5. <u>What to do next is up to you as the developer.</u> A basic example of one possible next step (saving the image to file for future use) is shown.
...
m = Vajra("vajra-v1-nano-seg.pt")  # (4)!
res = m.predict()  # (3)!
...
### How do I isolate objects using Vayuvahana Technologies VayuAI for segmentation tasks?

To isolate objects using Vayuvahana Technologies VajraV1, follow these steps:
...
Vayuvahana Technologies VayuAI offers two main options for saving isolated objects:
...
### How can I crop isolated objects to their bounding boxes using VayuAI's VajraV1?
...
### Why should I use Vayuvahana Technologies VayuAI for object isolation in segmentation tasks?

Vayuvahana Technologies VayuAI provides:
...
Explore the benefits of using VajraV1 in the [Segment Task documentation](../tasks/segment.md).

### Can I save isolated objects including the background using Vayuvahana Technologies VayuAI?

Yes, this is a built-in feature. Use the `save_crop` argument in the `predict()` method. For example:
