# Model Prediction with Vayuvahana Technologies VajraV1

## Introduction

In machine learning and computer vision, the process of using a trained model to run a single forward pass on an input image to make sense of the visual data (to detect and localize objects in the case of object detection) is called 'inference' or 'prediction'. Vayuvahana Technologies Private VayuAI offers the **predict mode** that is tailored for high-performance, real-time inference on a wide range of data sources.

## Why Use Vayuvahana Technologies VajraV1 for Inference?
...
VayuAI's predict mode is designed to be robust and versatile, featuring:
...
Vayuvahana Technologies VajraV1 models return either a Python list of `Results` objects, or a memory-efficient Python generator of `Results` objects when `stream=True` is passed to the model during inference:
...
!!! example "Predict"
    ...
    === "Return a list with `stream=False`"
        ...
        # Load a model
        model = Vajra("vajra-v1-nano-det.pt")  # pretrained VajraV1-nano model
    ...
    === "Return a generator with `stream=True`"
        ...
        # Load a model
        model = Vajra("vajra-v1-nano-det.pt")  # pretrained VajraV1-nano-det model
...
VajraV1 can process different types of input sources for inference, as shown in the table below. The sources include static images, video streams, and various data formats. The table also indicates whether each source can be used in streaming mode with the argument `stream=True` ✅. Streaming mode is beneficial for processing videos or live streams as it creates a generator of results instead of loading all frames into memory.
...
!!! example "Prediction sources"
    ...
    === "image"
        ...
        # Load a pretrained VajraV1-nano-det model
    ...
    === "Webcam"
        ...
        # Load a pretrained VajraV1 model
        model = Vajra("vajra-v1-nano-det.pt")
...
!!! note
    ...
    Vayuvahana Technologies VayuAI uses minimal padding during inference by default (`rect=True`). In this mode, the shorter side of each image is padded only as much as needed to make it divisible by the model's maximum stride, rather than padding it all the way to the full `img_size`. When running inference on a batch of images, minimal padding only works if all images have identical size. Otherwise, images are uniformly padded to a square shape with both sides equal to `img_size`.

    - `batch=1`, using `rect` padding by default.
    - `batch>1`, using `rect` padding only if all the images in one batch have identical size, otherwise using square padding to `img_size`.
...
VajraV1 supports various image and video formats, as specified in [vajra/dataset/utils.py](https://github.com/NamanMakkar/VayuAI/vajra/dataset/utils.py). See the tables below for the valid suffixes and example predict commands.

### Images

The below table contains valid image formats for prediction.
...
| -------------- | -------------------------------- | -------------------------------------------------------------------------- |
| `.bmp`         | `vajra predict source=image.bmp`  | [Microsoft BMP File Format](https://en.wikipedia.org/wiki/BMP_file_format) |
| `.dng`         | `vajra predict source=image.dng`  | [Adobe DNG](https://en.wikipedia.org/wiki/Digital_Negative)                |
| `.jpeg`        | `vajra predict source=image.jpeg` | [JPEG](https://en.wikipedia.org/wiki/JPEG)                                 |
| `.jpg`         | `vajra predict source=image.jpg`  | [JPEG](https://en.wikipedia.org/wiki/JPEG)                                 |
| `.mpo`         | `vajra predict source=image.mpo`  | [Multi Picture Object](https://fileinfo.com/extension/mpo)                 |
| `.png`         | `vajra predict source=image.png`  | [Portable Network Graphics](https://en.wikipedia.org/wiki/PNG)             |
| `.tif`         | `vajra predict source=image.tif`  | [Tag Image File Format](https://en.wikipedia.org/wiki/TIFF)                |
| `.tiff`        | `vajra predict source=image.tiff` | [Tag Image File Format](https://en.wikipedia.org/wiki/TIFF)                |
| `.webp`        | `vajra predict source=image.webp` | [WebP](https://en.wikipedia.org/wiki/WebP)                                 |
| `.pfm`         | `vajra predict source=image.pfm`  | [Portable FloatMap](https://en.wikipedia.org/wiki/Netpbm#File_formats)     |
| `.HEIC`        | `vajra predict source=image.HEIC` | [High Efficiency Image Format](https://en.wikipedia.org/wiki/HEIF)         |

### Videos

The table below contains valid video formats for the predict mode.

| Video Suffixes | Example Predict Command          | Reference                                                                        |
| -------------- | -------------------------------- | -------------------------------------------------------------------------------- |
| `.asf`         | `vajra predict source=video.asf`  | [Advanced Systems Format](https://en.wikipedia.org/wiki/Advanced_Systems_Format) |
| `.avi`         | `vajra predict source=video.avi`  | [Audio Video Interleave](https://en.wikipedia.org/wiki/Audio_Video_Interleave)   |
| `.gif`         | `vajra predict source=video.gif`  | [Graphics Interchange Format](https://en.wikipedia.org/wiki/GIF)                 |
| `.m4v`         | `vajra predict source=video.m4v`  | [MPEG-4 Part 14](https://en.wikipedia.org/wiki/M4V)                              |
| `.mkv`         | `vajra predict source=video.mkv`  | [Matroska](https://en.wikipedia.org/wiki/Matroska)                               |
| `.mov`         | `vajra predict source=video.mov`  | [QuickTime File Format](https://en.wikipedia.org/wiki/QuickTime_File_Format)     |
| `.mp4`         | `vajra predict source=video.mp4`  | [MPEG-4 Part 14 - Wikipedia](https://en.wikipedia.org/wiki/MPEG-4_Part_14)       |
| `.mpeg`        | `vajra predict source=video.mpeg` | [MPEG-1 Part 2](https://en.wikipedia.org/wiki/MPEG-1)                            |
| `.mpg`         | `vajra predict source=video.mpg`  | [MPEG-1 Part 2](https://en.wikipedia.org/wiki/MPEG-1)                            |
| `.ts`          | `vajra predict source=video.ts`   | [MPEG Transport Stream](https://en.wikipedia.org/wiki/MPEG_transport_stream)     |
| `.wmv`         | `vajra predict source=video.wmv`  | [Windows Media Video](https://en.wikipedia.org/wiki/Windows_Media_Video)         |
| `.webm`        | `vajra predict source=video.webm` | [WebM Project](https://en.wikipedia.org/wiki/WebM)    

## Working with Results

All VayuAI `predict()` calls will return a list of `Results` objects:
...
For more details see the [`Results` class documentation](../reference/core/results.md).
...
For more details see the [`Boxes` class documentation](../reference/core/results.md#vajra.core.results.Boxes).
...
!!! example "Masks"
    ...
    # Load a pretrained VajraV1-nano-seg Segment model
...
For more details see the [`Masks` class documentation](../reference/core/results.md#vajra.core.results.Masks).
...
!!! example "Keypoints"
    ...
    # Load a pretrained VajraV1-nano-pose Pose model
...
For more details see the [`Keypoints` class documentation](../reference/core/results.md#vajra.core.results.Keypoints).
...
!!! example "Probs"
    ...
    # Load a pretrained VajraV1-nano-cls Classify model
...
For more details see the [`Probs` class documentation](../reference/core/results.md#vajra.core.results.Probs).
...
!!! example "OBB"
    ...
    # Load a pretrained VajraV1-nano-obb model
...
For more details see the [`OBB` class documentation](../reference/core/results.md#vajra.core.results.OBB).
...
!!! example "Plotting"
    ...
    # Load a pretrained VajraV1-nano model
    model = Vajra("vajra-v1-nano-det.pt")
...
Ensuring thread safety during inference is crucial when you are running multiple VajraV1 models in parallel across different threads. Thread-safe inference guarantees that each thread's predictions are isolated and do not interfere with one another, avoiding race conditions and ensuring consistent and reliable outputs.

When using VajraV1 models in a multi-threaded application, it's important to instantiate separate model objects for each thread or employ thread-local storage to prevent conflicts:
...
!!! example "Thread-Safe Inference"
    ...
    def thread_safe_predict(model, image_path):
        """Performs thread-safe prediction on an image using a locally instantiated VajraV1 model."""
        model = Vajra(model)
    ...
    Thread(target=thread_safe_predict, args=("vajra-v1-nano-det.pt", "image1.jpg")).start()
    Thread(target=thread_safe_predict, args=("vajra-v1-nano-det.pt", "image2.jpg")).start()
    ```

## Streaming Source `for`-loop

Here's a Python script using OpenCV (`cv2`) and VajraV1 to run inference on video frames. This script assumes you have already installed the necessary packages (`opencv-python` and `vajra`).
...
!!! example "Streaming for-loop"
    ...
    while cap.isOpened():
        ...
        if success:
            # Run Vajra inference on the frame
            results = model(frame)
            ...
            cv2.imshow("VajraV1 Inference", annotated_frame)
...
### What is Vayuvahana Technologies VajraV1 and its predict mode for real-time inference?

Vayuvahana Technologies VajraV1 is a state-of-the-art model for real-tim object detection, segmentation, and classification. Its **predict mode** allows users to perform high-speed inference on various data sources such as images, videos and live streams. Designed for performance and versatility, it also offers batch processing and streaming modes. For more details on its features, check out the [Vayuvahana Technologies VayuAI predict mode](#key-features-of-predict-mode).
