# TFLite for VajraV1 Model Export
...
The TFLite export allows you to optimize your VajraV1 models for tasks like object detection in edge device based applications. In this guide, we will walk throught the steps for converting your models to the TFLite format, making it easier for your models to perform well on various edge devices.
...
TensorFlow Lite is compatible with a wide range of platforms, including embedded Linux, Android, iOS, and MCU. Exporting your modle to TFLite makes your applications faster, more reliable, and capable of running offline.
...
Before we look at the code for exporting VajraV1 models to the TFLite format, let's understand how TFLite models are normally used. 
...
## Export to TFLite: Converting your VajraV1 Model
...
All VajraV1 models are designed to support export out of the box, making it easy to integrate them into your preferred deployment workflow. You can [view the full list of supported export formats and configuration options](../modes/export.md) to choose the best setup for your application.
...
!!! example "Usage"
    ...
    === "Python"
          ...
          model.export(format="tflite")  # creates 'vajra-v1-nano-det_float32.tflite'

          # Load the exported TFLite model
          tflite_model = Vajra("vajra-v1-nano-det_float32.tflite")

          # Run inference
          results = tflite_model("path/to/img.jpg")
          ```

    === "CLI"

          ```bash
          # Export a VajraV1 PyTorch model to TFLite format
          vajra export model=vajra-v1-nano-det.pt format=tflite # creates 'vajra-v1-nano-det_float32.tflite'

          # Run inference with the exported model
          vajra predict model='vajra-v1-nano-det_float32.tflite' source='path/to/img.jpg'
          ```
...
For more details on the export process, visit the [documentation page on exporting](../modes/export.md)

## Deploying the Exported VajraV1 TFLite Models

After successfully exporting your VajraV1 models to TFLite format, you can now deploy them. The primary and recommended first step for running a TFLite model is to utilize the Vajra(model.tflite) method as outlined in the previous usage code snippet. However, for in-depth instructions on deploying your TFLite models in various other settings, take a look at the following resources:
...
In this guide we focused on how to export to the TFLite format. By converting your VajraV1 models to TFLite model format, you can improve the efficiency of VajraV1 models, making them more effective and suitable for edge computing environments.

For further details on usage, visit the [TFLite official documentation](https://ai.google.dev/edge/litert).

## FAQ

### How do I export a VajraV1 model to TFLite format?

To export a VajraV1 model to TFLite format, you can use the VayuAI library. First, install the required package using:
...
For more details, visit the [Vayuvahana Technologies VayuAI export guide](../modes/export.md).

### What are the benefits of using TensorFlow Lite for VajraV1 model deployment?

TFLite is an open-source deep learning framework designed for on-device inference, making it ideal for deploying VajraV1 models on mobile, embedded, and IoT devices. Key benefits include:
...
### Can I use TFLite models on microcontrollers for VajraV1 predictions?

Yes, TFLite supports deployment on microcontrollers with limited resources. TFLite's core runtime requires only 16 KB of memory on an Arm Cortex M3 and can run basic VajraV1 models. This makes it suitable for deployment on devices with minimal computational power and memory.
...
### What platforms are compatible with TFLite exported VajraV1 models?

TensorFlow Lite provides extensive platform compatibility, allowing you to deploy VajraV1 models on a wide range of devices, including:
...
### How do I troubleshoot common issues during VajraV1 model export to TFLite?

If you encounter errors while exporting VajraV1 models to TFLite, common solutions include:

- **Check package compatibility**: Ensure you're using compatible versions of VayuAI and TensorFlow. 
- **Model support**: Verify that the specific VajraV1 model supports TFLite export by checking the VayuAI [export documentation page](../modes/export.md).
